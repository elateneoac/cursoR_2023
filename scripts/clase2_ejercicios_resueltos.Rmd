---
title: "Introducción al análisis de datos con R: Clase 2 - Ejercitación Resuelta"
...
buenos_aires <- barrios_populares %>%
...
df_buenos_aires <- buenos_aires %>% 
  select(`Departamento \\ Partido`, Electricidad, Agua,Cloaca)        


# muestro las primeras diez filas
head(df_buenos_aires)
...
buenos_aires %>% 
  ...
  arrange(desc(cantidad)) #lo ordeno
...
# porcentaje total
buenos_aires %>% 
  summarise(porcentaje = round((sum(familias_encuestadas)/sum(`Familias estimadas`)),2)*100)
...
#renombro la variable ´familias estimadas´

buenos_aires <- buenos_aires %>% 
  rename(familias_estimadas = `Familias estimadas`)
...
write.csv(buenos_aires, paste0("../data/", clase, "/buenos_aires.csv"), row.names = F, fileEncoding = "UTF-8")
```
